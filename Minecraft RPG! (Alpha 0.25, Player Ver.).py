...
try:
    ...
    raise ModuleNotFoundError("НЕ ЗНАЙДЕНО ФАЙЛ DCOLA.PY!!!")
import random
import importlib
...
while True:
    ...
    if c == "t":
        ...
        if player["y"] == 0:
            if map[0][player["x"]] == water:
                beep("nomove")
            else:
                player["y"] = len(map[0]) - 1
        print(player["y"])
        if not player["y"] == 0:
        ...
        if player["x"] == 0:
            if map[player["y"]][0] == water:
                beep("nomove")
            else:
                player["x"] = len(map[0]) - 1
